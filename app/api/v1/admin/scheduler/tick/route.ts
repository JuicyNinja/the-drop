...
import { enqueueWindowClosing, enqueueNearlyGone } from "@/lib/notifications";
...
const route = defineRoute(
  {
    ...
    response: {
      data: z.object({
        ...
        reconciled: z.number(),
        pressure: z.number(),
        notifications_enqueued: z.number(),
      }),
    },
    errors: ["FORBIDDEN"],
  },
  async ({ user }) => {
    ...
    // Time/threshold-based notification scans — ENQUEUE only (the dispatcher
    // sends). Window-closing fires ~2h before redeem_until; nearly-gone at ~85%
    // claimed. Both idempotent (dedup per drop).
    const wc = await enqueueWindowClosing();
    const ng = await enqueueNearlyGone();
    return { data: { went_live: live.went_live, gone: closed.gone, expired: closed.expired, reconciled, pressure, notifications_enqueued: wc.enqueued + ng.enqueued } };
  },
);
