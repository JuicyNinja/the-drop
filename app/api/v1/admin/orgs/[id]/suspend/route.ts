...
import { suspendOrg } from "@/lib/admin/orgs";

/** Suspend an org (admin). Audited. */
const route = defineRoute(
  {
    method: "post",
    path: "/v1/admin/orgs/{id}/suspend",
    operationId: "adminSuspendOrg",
    summary: "Suspend an org (admin)",
    tags: ["Admin"],
    auth: "user",
  ...
  async ({ user, params, request }) => {
    ...
    return { data: await suspendOrg({ actorId: user.id, ip: ipFromRequest(request) }, params.id) };
  },
);
