...
11. **Dev-mode green does not mean production works; a gate whose subject is a build-time or environment concern must run against `npm run build`, not the dev server.** The strict CSP (`middleware.ts`) puts a per-request nonce on `script-src … 'strict-dynamic'`, but the buyer pages are client components with no dynamic opt-in, so a production build prerendered them **static** — their bootstrap `<script>` tags carried no runtime nonce and were all blocked. Four of the six buyer routes (`/`, `/wallet`, `/redeem`, `/you`; `/graveyard` likewise — only `/drops/[id]` and `/merchants/[id]` render dynamically) were therefore **inert in production**: no catch, no filters, no follow, no redeem. Every WP gate reported green throughout, because the gates run against the dev server and **dev never prerenders**, so the CSP-plus-static-generation interaction that exists only in a production build was never exercised. It surfaced only when a production build was checked directly (`next start` + a header-nonce-vs-script-nonce comparison). The fix was one line — `export const dynamic = "force-dynamic"` on the shared `(ui)` layout — but the miss was structural: the gate certified a path production does not take. The rule, once: **any gate whose subject is CSP, hydration, security headers, fonts, caching, or environment variables is verified against `npm run build` / `next start` output, never the dev server alone, because the dev and production paths for those subjects differ and a dev-only pass certifies the wrong path.** Its release-gate corollary is a standing v1 item (§ v1 RELEASE GATE): a production build must be proven to hydrate on every route.
...
- [ ] **Production build hydrates on every route** (rule 11) — verified against `npm run build` + `next start`, not the dev server. For every route: the response CSP header nonce matches the nonce on the served `<script>` tags, **and** one interactive control is confirmed working (a catch, a filter, a follow, the redeem keypad — an actual interaction), not merely a 200. A dev-only pass does not satisfy this item.
